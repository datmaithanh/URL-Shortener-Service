...
func (server *Server) getUrl(ctx *gin.Context) {
	...
	url, err := server.store.GetUrl(ctx, req.ID)
	if err != nil {
		ctx.JSON(http.StatusNotFound, errorResponse(err))
		return
	}
	urlResponse := urlsResponse{
		Code:        url.Code.String,
		ShortURL:    url.ShortUrl.String,
	...
	ctx.JSON(http.StatusOK, urlResponse)
}
...
func (server *Server) listUrls(ctx *gin.Context) {
	...
	urls, err := server.store.ListUrl(ctx, db.ListUrlParams{
		Limit:  req.PageSize,
